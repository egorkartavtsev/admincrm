...
<?php
class ControllerProductionCatalog extends Controller {
	private $error = array();
	...
	public function delete() {
      ...
      if (isset($this->request->get['filter_category'])) {
        $url .= '&filter_category=' . $this->request->get['filter_category'];
			...
			$this->response->redirect($this->url->link('production/catalog', '1' . $url, true));
		}
	...
	protected function getList() {
		...
		$data['delete'] = $this->url->link('production/catalog/delete', '1' . $url, true);
...
//			}
                        ...
                        if($result['product_id']!=NULL){
                            $data['products'][] = array(
                                    ...
                                    'status'     => $stat,
                                    'edit'       => $this->url->link('production/catalog/edit', 'product_id=' . $result['product_id'] . $url, true)
                            );
                        }       
		}  
		...
		$data['sort_name'] = $this->url->link('production/catalog', 'sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('production/catalog', 'sort=p.length' . $url, true);
		$data['sort_manager'] = $this->url->link('production/catalog', 'sort=p.manager' . $url, true);
		$data['sort_locate'] = $this->url->link('production/catalog', 'sort=p.weight' . $url, true);
		$data['sort_date_added'] = $this->url->link('production/catalog', 'sort=p.date_added' . $url, true);
		$data['sort_price'] = $this->url->link('production/catalog', 'sort=p.price' . $url, true);
		$data['sort_category'] = $this->url->link('production/catalog', 'sort=p2c.category_id' . $url, true);
		$data['sort_quantity'] = $this->url->link('production/catalog', 'sort=p.quantity' . $url, true);
		$data['sort_status'] = $this->url->link('production/catalog', 'sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('production/catalog', 'sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('production/catalog', '1' . $url . '&page={page}', true);
...
//            $data['uName'] = $this->session->data['username'];
            ...
            $data['action'] = $this->url->link('production/catalog/saveForm', 'product_id='.$this->request->get['product_id'].$url);
            $data['goBack'] = $this->url->link('production/catalog'.$url);
            $local_id = 0;
            foreach($photos as $img){
                $data['images'][] = array(
                        'image'         => $img['image'],
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'production/catalog')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
    ...
    public function saveForm() {
        ...
        $this->response->redirect($this->url->link('production/catalog'.$url));
    }
